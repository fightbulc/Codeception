<?php
    ...
    class JSONRPC extends \Codeception\Module
    {
        ...
        public function _before(\Codeception\TestCase $test)
        {
            if (!$this->client)
            {
                ...
                else
                {
                    ...
                    $this->client = $this->getModule('PhpBrowser')->session->getDriver()
                        ->getClient();
                }
                if (!$this->client)
                {
        ...
        {
            \PHPUnit_Framework_Assert::assertEquals($num, $this->client->getResponse()
                ->getStatus());
        }
        ...
        {
            $result = $this->_decodeJson($this->response);
         ...
         * @param int $id
         */
        public function sendJSONRPCMethodCall($id, $methodName, $parameters = array())
        {
        ...
        {
            $this->_decodeJson($this->response);
        ...
        {
            $data = $response = $this->_decodeJson($this->response, TRUE);
